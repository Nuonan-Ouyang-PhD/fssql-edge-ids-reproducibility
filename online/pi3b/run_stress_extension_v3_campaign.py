#!/usr/bin/env python3
"""Execute the frozen Stress Extension V3 matrix and stop on invalid evidence."""
...
def main() -> None:
    ...
    if len(rows) != expected_runs or [int(row["global_order"]) for row in rows] != list(range(1, expected_runs + 1)):
    ...
    with progress_path.open("x", newline="", encoding="utf-8", buffering=1) as stream:
        ...
        for row in rows:
            ...
            command = [
                sys.executable,
                str(ROOT / "online/pi3b/collect_policy_run.py"),
            ...
            if result.returncode or status != "FORMAL_POLICY_RUN_PASS":
                (args.output / "CAMPAIGN_STOP.json").write_text(json.dumps({
                    "status": "STRESS_EXTENSION_V3_STOPPED_ON_INVALID_ATTEMPT",
    ...
    (args.output / "CAMPAIGN_COMPLETE.json").write_text(json.dumps({
        "status": "STRESS_EXTENSION_V3_POLICY_MATRIX_COMPLETE",
        "runs": len(rows),
